  ...
  return (
    <>
      ...
      <Drawer title="用户详情" open={open} onClose={() => setOpen(false)} width={360}>
        <p style={{ color: "var(--text-soft)" }}>手机号、角色、最后登录时间</p>
      </Drawer>
    </>
  );
}
...
const components = [
  {
    ...
    render: () => (
      <div style={{ border: "1px solid var(--paper)", borderRadius: 12, padding: 16, maxWidth: 320, background: "#fff" }}>
        <Space align="start" size={12}>
          <Avatar size={48} style={{ backgroundColor: "var(--red)" }}>张</Avatar>
          <div>
            <div style={{ fontWeight: 700 }}>张三</div>
            <div style={{ color: "var(--svg-purple)", marginBottom: 8 }}>zhangsan@example.com</div>
            <Space size={6}>
  ...
  },
  {
    ...
    render: () => (
      <Layout style={{ minHeight: 64, borderRadius: 12, overflow: "hidden" }}>
        <Header style={{ display: "flex", alignItems: "center", gap: 24, padding: "0 24px", background: "#fff", borderBottom: "1px solid var(--paper)" }}>
          <div style={{ fontWeight: 700, color: "var(--red)" }}>Logo</div>
  ...
  },
  {
    ...
    render: () => (
      <Layout style={{ minHeight: 240, borderRadius: 12, overflow: "hidden", background: "#fff" }}>
        <Sider width={180} style={{ background: "#fff", borderRight: "1px solid var(--paper)" }}>
        ...
        <Content style={{ padding: 24, color: "var(--svg-purple)", background: "var(--paper-card)" }}>主内容区</Content>
      </Layout>
    ),
  },
  {
    ...
    render: () => {
      const columns = [
        { title: "姓名", dataIndex: "name", key: "name", render: (t) => <Space><Avatar size="small" style={{ backgroundColor: "var(--red)" }}>{t[0]}</Avatar>{t}</Space> },
  ...
  },
  {
    ...
    render: () => (
      <Space direction="vertical" style={{ width: "100%" }} size="large">
        <div>
          <div style={{ color: "var(--svg-purple)", marginBottom: 8 }}>Loading 态：</div>
          <Spin tip="加载中..."><div style={{ height: 60, padding: 20, background: "var(--paper-card)", borderRadius: 8 }} /></Spin>
        </div>
        <div>
          <div style={{ color: "var(--svg-purple)", marginBottom: 8 }}>Empty 态：</div>
...
export function ComponentsPage({ currentPath, prev, next }) {
  ...
  return (
    <PageFrame title="组件认知页 · 后端同学的前端课程">
      <ConfigProvider theme={{ token: { colorPrimary: "var(--red)", borderRadius: 8, fontFamily: "inherit" } }}>
