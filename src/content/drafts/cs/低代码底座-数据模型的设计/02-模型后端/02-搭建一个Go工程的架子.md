...
解决什么问题：
1. 屏蔽具体的日志实现细节。
2. 同一日志格式。通过包装的方式，保证AI使用同一格式的日志。
3. 增强对error即错误的处理。 
...
这个事情不是“体验差一点”那么简单，我遇到过非常真实、也非常离谱的线上事故场景：

**服务已经出故障了，日志里也看到 error 了，但就是没有堆栈。**

这时候你本来最需要做的事情，应该是直接定位问题、直接修问题。

结果现实变成了什么？

**你得先发一版新镜像，只为了把堆栈补打出来，然后再看下一轮日志。**

我每次遇到这种事情都很无力吐槽。因为这说明工程在最该帮你的时候，完全没帮上忙。

线上故障排查本来就争分夺秒，如果“看堆栈”这件事情还要靠重新发版才能拿到，那说明你的错误系统从一开始就设计错了。
...
结合我自己在 `modelcraft-backend/pkg/bizerrors` 里的实现，我现在更喜欢把错误拆成两层：

> 真实代码入口：[`luker1228/modelcraft/tree/master/modelcraft-backend/pkg/bizerrors`](https://github.com/luker1228/modelcraft/tree/master/modelcraft-backend/pkg/bizerrors)
...
在古法编程时代，我不会推给你DDD的设计。 太麻烦了，太啰嗦了。但是在AI时代，很多adapter代码太廉价了。我强推DDD。至于具体的设计，其实我特别推荐极客时间 钟敬老师的《手把手教你落地DDD》
...
func (r *MySQLRepository) FindMany(ctx context.Context, input modelruntime.FindManyInput) ([]modelruntime.Record, error) {
    ...
    if err != nil {
        return nil, bizerrors.WrapError(err, bizerrors.SystemError, "build query failed")
    }

    rows, err := r.db.QueryContext(ctx, query, args...)
    if err != nil {
        return nil, mapDBError(ctx, err, input.ModelID)
    }
    defer rows.Close()
