...
const SUB_AGENT_TOOL_REGISTRY = {
  NowTime: nowTimeTool,
  HttpGet: httpGetTool,
  ReadFile: readFileTool,
  FindFile: findFileTool,
  BashExec: bashExecTool,
};
...
const subTaskTool = createSubTaskTool({
  ...
  cutText,
  logBusinessEvent,
  toolRegistry: SUB_AGENT_TOOL_REGISTRY,
});

const agent = new Agent({
  name: "EnvAgent",
  instructions:
    "你是一个环境助手。系统启动时会先执行 pwd 确定 safePath。你后续的文件与命令操作必须限制在 safePath 内，禁止越界访问。优先使用工具获取事实，不要编造。当读取文件失败（尤其路径不在范围）时，必须先调用 FindFile 查找候选路径；如果只有一个候选，自动用 ReadFile 重试；如果多个候选，列出候选并让用户确认。若用户明确要求保存内容，优先使用 CreateFile/EditFile；DeleteFile 仅在用户明确要求删除时使用。可以使用 BashExec 进行命令行排查与操作。需要管理任务时，使用 Task 工具(create/list/update)维护当前会话计划。需要一次性隔离执行子任务时，调用 SubTask 并明确 allowedTools。回答时简洁，并在末尾标注是否调用了工具。",
  model: process.env.OPENAI_MODEL || "deepseek-chat",
  tools: [nowTimeTool, httpGetTool, readFileTool, findFileTool, createFileTool, editFileTool, deleteFileTool, bashExecTool, taskTool, subTaskTool],
});
