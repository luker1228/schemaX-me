...
画法：左侧输入，中间 Luke 或怪机器处理，右侧输出，橙色箭头表达主流向。

### 系统局部

适合：信息来源、过滤器、数据库、渲染器、agent 系统局部。

画法：只画 3-5 个核心模块，Luke 参与其中一个关键动作。
...
画法：一层层盒子，不要正式金字塔；Luke 在旁边搬砖或搭建。

### 地图路线

适合：从想法到上线、用户路径、内容承接路径、学习路线。

画法：一条弯曲路径，少量节点，Luke 牵线或走路。
...
3. 让 Luke 承担动作：不是站旁边，而是卡在机器里、拉错线、守门、搬运、修补、称重、扶梯子、记录、把东西塞进某个怪装置。
...
不要默认打开或复刻 `assets/examples/`。这些图片只用于风格校准，例如线条密度、留白、颜色克制、Luke 气质。
...
同类主题也要换新隐喻。例如“承接路径”不一定画路线，可以画 Luke 把内容尾巴接到门把手；“一鱼多吃”不一定画鱼，可以画 Luke 把一个纸团压成几种形状。
